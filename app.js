/* Pong2025 — iPhone-optimized PWA (single-folder) */
(() => {
  'use strict';
  ...
  // --- Virtual base resolution (logic space) ---
  const BASE_W = 800, BASE_H = 450;
  let scaleX = 1, scaleY = 1;

  // --- Stable sizing (debounced), DPR capped for iOS fill-rate ---
  function applySize() {
    const DPR = Math.min(2, window.devicePixelRatio || 1); // cap to 2
  ...
  function setMode(m) { state.mode = m; localStorage.setItem('pong_mode', m); }
  ...
  // --- Fixed timestep loop (smoother on iOS) ---
  const STEP = 1000/60;
  let last = performance.now(), acc = 0;
  function loop(ts){
    acc += ts - last; last = ts;
    while (acc >= STEP) {
      if (!state.paused && menuEl.style.display === 'none') {
        // keyboard controls
