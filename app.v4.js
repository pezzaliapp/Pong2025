/* Pong2025 v4 — menu on start, portrait helper, quality toggle, iPhone-friendly */
(() => {
  'use strict';
  ...
  function init(){
    ...
    const qLowBtn = document.getElementById('qLowBtn');
    const qMidBtn = document.getElementById('qMidBtn');
    const qHighBtn= document.getElementById('qHighBtn');
    ...
    const cfg = {
      ...
      quality: localStorage.getItem('pong_quality') || 'mid', // low|mid|high => DPR 1/1.5/2
      paused: false
    };

    function dprCap(){
      return cfg.quality === 'low' ? 1 : (cfg.quality === 'high' ? 2 : 1.5);
    }

    // --- Logic space is fixed landscape (800x450). We'll fit it in portrait by shrinking width. ---
    const BASE_W = 800, BASE_H = 450;
    let scaleX = 1, scaleY = 1;

    function isMobile(){ return Math.max(window.innerWidth, window.innerHeight) < 900; }
    function applySize(){
      // Portrait helper on mobile: if device is landscape, show rotate hint
      if (isMobile()){
      ...
      // Fit canvas inside viewport with margins, prioritizing portrait devices
      let vw = Math.min(window.innerWidth, 1000);
      let vh = Math.min(window.innerHeight, 820);
      // Ensure canvas height doesn't intrude under HUD — reserve ~60px
      vh -= 60;
      // Maintain 16:9 landscape inside portrait viewport
    ...
    function setSpeed(s){ cfg.speed = s; localStorage.setItem('pong_speed', s);
    ...
    function setQuality(q){ cfg.quality = q; localStorage.setItem('pong_quality', q); applySize(); }
    ...
    const keys = new Set();
    window.addEventListener('keydown', e=> { keys.add(e.key.toLowerCase()); if (e.key==='Escape') menuEl.style.display='grid'; });
    ...
    function draw(){
      ...
      const dash = 16;
      for (let y=10; y<BASE_H-10; y+=dash*1.6){ ctx.fillRect(BASE_W/2 - 2, y, 4, dash); }
